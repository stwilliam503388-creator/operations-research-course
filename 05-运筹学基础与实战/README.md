...
> Sidebar 章节 `SB1`~`SB4` 是工具补课：读主线时遇到 Pyomo、仿真、元启发式或产品组合建模卡住，再回来看对应 sidebar，不必一开始全读。
...
本课程是整套优化课程的主干：先学会“业务问题 → 决策变量 → 约束 → 目标函数”，再根据问题类型进入不同专题。

| 相关课程 | 推荐衔接方式 |
|----------|----------------|
| [低复杂度算法设计](../02-低复杂度算法设计/) | 先补算法复杂度、DP、贪心和图算法，再回来看网络流与启发式 |
| [MIP求解器技术](../04-MIP求解器技术/) | 学完 LP/MIP 基础后，继续理解分支定界、割平面和求解器日志 |
| [库存与供应链管理](../07-库存与供应链管理/) | 把库存、补货、牛鞭效应和网络设计作为运筹学的行业落地线 |
| [随机规划与鲁棒优化](../08-随机规划与鲁棒优化/) | 当需求、价格、供应不确定时，把确定性模型升级为不确定性优化 |
| [多目标优化](../09-多目标优化/) | 当成本、服务、风险、碳排不能合成一个目标时，学习 Pareto 权衡 |
| [凸优化与非线性优化](../10-凸优化与非线性优化/) | 当变量连续、目标非线性或要理解 KKT/梯度法时，补连续优化基础 |
