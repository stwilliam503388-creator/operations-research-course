...
园丁要培育新花，既想花瓣大，又想花期长，还不希望植株太脆弱。年轻学徒每代只留下“总分最高”的几株，几轮之后花园里几乎全是同一种花：某一项很好，其他方向的可能性却消失了。

老园丁换了办法。他先把被其他花全面压过的苗淘汰，再把剩下的苗按层次摆开；同一层里，还要保留彼此差异较大的苗，避免花园只挤在一个角落。下一代从这些苗中杂交、变异，继续筛选。学徒这才明白，多目标进化不是尽快收敛到一个冠军，而是在前沿上保留多样性。

故事快结束时，选苗过程露出 NSGA-II 的名字。非支配排序帮助判断谁处在更好的前沿层，拥挤距离帮助维持候选解分布。

> 停一下，想一想：如果只留下总分最高的个体，为什么可能找不到完整 Pareto 前沿？

这个故事里，花苗对应候选解，花瓣/花期/强健度对应多个目标，层次摆放对应非支配排序，保留差异对应拥挤距离。NSGA-II 用选择、交叉、变异和精英保留逐步逼近多目标前沿。
## 场景描述
...
完整代码在 `code/python/case07_nsga2.py`，约 370 行。核心组件：
...
运行 `python code/python/case07_nsga2.py`：
...
如果你想进一步做可视化，可以在 `code/python/case07_nsga2.py` 的输出基础上扩展下面几类图：
